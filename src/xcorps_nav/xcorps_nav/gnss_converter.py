...
class GNSSConverter(Node):
    def __init__(self):
        ...
        default_params = {
            ...
            'wp' : [37.4557513333, 126.9516513333],
        }
        ...
        self.wp = self.declare_parameter("wp", default_params['wp']).value
        ...
        self.wp_timer = self.create_timer(0.1, self.wp_pub)
    ...
    def OS_pub_enu_pos(self):
    	if self.OS_gps_lat_received == True:
            self.OS[0], self.OS[1], self.OS[2] = self.enu_convert([self.OS_gps_lat, self.OS_gps_lon])
            enu_pos = Point()
            enu_pos.x = self.OS[0]
            enu_pos.y = self.OS[1]
    ...
    def wp_pub(self):
        ...
        #    self.enu_wp_set[i, 0], self.enu_wp_set[i, 1], dummy = self.enu_convert(self.wp_set[i, 0], self.wp_set[i, 1])
         
        self.enu_wp[0], self.enu_wp[1], self.enu_wp[2] = self.enu_convert([self.wp[0], self.wp[1]])    

        
        enu_wp = Point()
        enu_wp.x = self.enu_wp[0]
        enu_wp.y = self.enu_wp[1]
        self.enu_wp_pub.publish(enu_wp)
        
    def TS_pub_enu_pos(self):
        if self.TS_gps_lat_received == True:
            self.TS[0], self.TS[1], self.TS[2] = self.enu_convert([self.TS_gps_lat, self.TS_gps_lon])
            enu_pos = Point()
            enu_pos.x = self.TS[0]
            enu_pos.y = self.TS[1]
