...
def dataSend_pos(latitude, longitude):
    data_pos = str(str(latitude) + '/' + str(longitude))
    while True:
        try:
            f1 = open(FILEPATH_pos, 'w')
            f1.write(data_pos)
            f1.close()
            break
        except:
...
def dataSend_spd(speed):
    data_spd = str(speed)
    while True:
        try:
            f2 = open(FILEPATH_spd, 'w')
            f2.write(data_spd)
            f2.close()
            break
        except:
